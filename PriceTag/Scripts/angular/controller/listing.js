...
function CategoryCtrl($scope, $q, $filter, $location, CommonFactory, ListingFactory) {
    ...
    $scope.myPromise = $q.all([CommonFactory.getCategory(), CommonFactory.getReference(3)]).then(function (result) {
    ...
    $scope.selectCategoryLv1 = function (id) {
        ...
        angular.forEach($scope.categories, function (v, k) {
            ...
            if (v.id === id) {
                ...
                console.log($scope.refenceListType);
    ...
    $scope.submit = function () {
        ...
        $scope.myPromise = $q.all([ListingFactory.create($scope.form)])
...
function ListingImageCtrl($scope, $q, ImageFactory, $filter, uploadManager, $route, $routeParams, $window) {
    ...
    ////Retrieve Imge
    //$scope.myPromise = $q.all([ImageFactory.get($scope.id)]).then(function (results) {
    ...
    $scope.$on('fileAdded', function (e, call) {
        ...
        reader.onload = function (e) {
            ...
            if ($scope.totalFiles > maxFile) {
                $scope.warning('Cannot upload more than ' + maxFile + ' photos.');
    ...
    $scope.$on('uploadCallBack', function (e, response) {
