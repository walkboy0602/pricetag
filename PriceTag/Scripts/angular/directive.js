...
adsApp.directive('ngUpload', ['uploadManager', function factory(uploadManager) {
    return {
        restrict: 'A',
        link: function (scope, element, attrs) {
            $(element).fileupload({
                ...
                add: function (e, data) {
                    ...
                    if (data.originalFiles[0]['size'].length || data.originalFiles[0]['size'] > 2000000) {
                        uploadErrors.push('Filesize is too big, maximum file size allow is 2mb.');
                    }
